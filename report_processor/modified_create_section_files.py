...
parser.add_argument(
    "--no_split", action="store_true", help="Do not output batched CSV files."
)
...
def main(args):
    ...
    for p_grp in p_grp_folders:
        ...
        for p in tqdm(p_folders):
            ...
            for s in studies:
                ...
                # NOTE: we will take the longest section as the section
                # that provides the best description of the image.
                # The aim is to obtain a factual description of the image,
                # rather than a diagnosis or conclusion
                ...
                # find the index for the findings and the report
                indication_idx = -1
                report_idx = -1

                if "indication" in section_names:
                    indication_idx = section_names.index("indication")
                elif "history" in section_names:
                    indication_idx = section_names.index("history")
                ...
                for sn in ("impression", "findings", "last_paragraph"):
                    if sn in section_names:
                        ...
                        curr_idx = section_names.index(sn)
                        if len(sections[curr_idx]) > length:
                            length = len(sections[curr_idx])
                            report_idx = curr_idx
                ...
                if report_idx == -1:
                    ...
                    p_study.append(sections[report_idx].strip())

                if indication_idx == -1:
                    p_study.append("")
                else:
                    p_study.append(sections[indication_idx].strip())
                patient_studies.append(p_study)

                study_sectioned = [s_stem]
                for sn in ("impression", "findings", "last_paragraph", "comparison", "indication"):
                    if sn in section_names:
                        idx = list_rindex(section_names, sn)
    ...
    if len(patient_studies) > 0:
        ...
        with open(
            output_path / "modified_mimic_cxr_sectioned_with_indication.csv", "w"
        ) as fp:
            csvwriter = csv.writer(fp)
            # write header
            csvwriter.writerow(
                ["study","impression","findings","last_paragraph","comparison","indication"]
            )
        ...
        if args.no_split:
            ...
            with open(output_path / f"modified_mimic_cxr_sections_with_indication.csv", "w") as fp:
                csvwriter = csv.writer(fp)

                csvwriter.writerow(["study", "report", "indication"])
            ...
            while n < len(patient_studies):
                n_fn = n // jmp
                with open(
                    output_path / f"modified_mimic_cxr_{n_fn:02d}.csv", "w"
                ) as fp:
                    csvwriter = csv.writer(fp)
                    csvwriter.writerow(["study", "report", "indication"])
